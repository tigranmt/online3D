...
this.openDropboxDrive=function(){var options={linkType:"direct",success:function(files){console.log(files);var appKey={key:'254hrzt7wfhcmxv',secret:'i9f2dtdx5y67wui',sandbox:true};var client=new Dropbox.Client(appKey);client.authDriver(new Dropbox.Drivers.Popup());client.authenticate(function(error,client){if(error){console.log(error);}
if(client.isAuthenticated()){for(var i=0;i<files.length;i++){var file=files[i];client.readFile(file.link,function(error,data){if(error){return console.log(error);}
alert(data);});}}else{console.log("Error on authentication");}});},cancel:function(){}};Dropbox.choose(options);}
