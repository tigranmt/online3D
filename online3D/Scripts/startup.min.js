...
﻿var startup=new(function(){"use strict";window.indexedFiles.openbase();var _this=this;var rootMap={"STL":"/Stl/StlView"}
...
this.openDropboxDrive=function(){var filesLoadingStarted=function(){showProgress(true);}
var fileLoadingFinished=function(files){showProgress(false);loadFiles(files);}
...
function loadFilesFromRemoteDriveAsync(files){if(files.length===0)
return;var index=files.length;var error=false;(function loadSingleFile(){index--;if(index<0){if(!error){var f0=files[0];var extension=f0.name.split('.').pop();var path=rootMap[extension.toUpperCase()];if(path){window.location.replace(path);}}
return;}
var f=files[index];var fileData={"fileName":f.name,"fileSize":f.size,"fileData":f.data}
if(addToStore(fileData,loadSingleFile)===false){toastr.error('Failed to load file '+f.name,'Error');showProgress(false);window.indexedFiles.deletebase();index--;error=true;return;}})();}
function loadFiles(files){if(!compatibilityChecks())
return;if(files.length==0)
return;if(files){showProgress(true);var index=files.length;var runLoad=function(){index--;if(index<0){(function(file){var extension=file.name.split('.').pop();var path=rootMap[extension.toUpperCase()];if(path){window.location.replace(path);}})(files[0]);}
...
else{toastr.error('Can not initialize GL engine','Error');showProgress(false);}};reader.onerror=function(event){console.error("File could not be read! Error code: "+event.target.error.code);toastr.error('Failed to load file '+file.name+' Error: '+event.target.error.toString(),'Error');showProgress(false);};if(file.blob!==undefined){reader.readAsBinaryString(file.data);}
else{reader.readAsBinaryString(file);}};runLoad();}
else{toastr.error('Can not load file: '+file.name,'Error');}}
function loadFileFromDrop(event){var files=event.dataTransfer.files;if(extensionIsOk(files)){loadFiles(files);}
else{toastr.error('Not supported file format','Error');}}
function loadFileFromDisk(event){var files=event.target.files;if(extensionIsOk(files)){loadFiles(files);}
else{toastr.error('Not supported file format','Error');}}
...
complete(false);}});}
$(document).ready(function(){if(compatibilityChecks()){setupDragDropEvents();setupFileLoadEvents();_this.loadSampleModelPreview();userAccess.UpdateUI();$('#access').on('click',function(){userAccess.requestUserAuth();});$("#introImageSrc").lazyload();$("#iconImageSrc").lazyload();}});})