...
this.openDropboxDrive=function(){var options={linkType:"direct",success:function(files){console.log(files);var appKey={key:'254hrzt7wfhcmxv',secret:'i9f2dtdx5y67wui',sandbox:true};var client=new Dropbox.Client(appKey);for(var i=0;i<files.length;i++){var file=files[i];client.readFile(file.name,function(error,data){if(error){return console.log(error);}
alert(data);});}},cancel:function(){}};Dropbox.choose(options);}
