...
else{var bounds=TOOLS.getSceneBoundingBox();var diag=new THREE.Vector3();diag=diag.subVectors(bounds.max,bounds.min);var diagLength=diag.length();var axis=new THREE.AxisHelper(diagLength);axis.arrow=true;this.glScene.add(axis);var shapex=new THREE.TextGeometry("X",{font:'helvetiker',height:0.5,size:2});var wrapperx=new THREE.MeshNormalMaterial({color:0x111111});var xAxis=new THREE.Mesh(shapex,wrapperx);xAxis.position=new THREE.Vector3(diagLength-2,0,0);xAxis.arrow=true;this.glScene.add(xAxis);var shapey=new THREE.TextGeometry("Y",{font:'helvetiker',height:0.5,size:2});var wrappery=new THREE.MeshNormalMaterial({color:0x111111});var yAxis=new THREE.Mesh(shapey,wrappery);yAxis.position=new THREE.Vector3(0,diagLength-2,0);yAxis.arrow=true;this.glScene.add(yAxis);var shapez=new THREE.TextGeometry("Z",{font:'helvetiker',height:0.5,size:2});var wrapperz=new THREE.MeshNormalMaterial({color:0x111111});var zAxis=new THREE.Mesh(shapez,wrapperz);zAxis.position=new THREE.Vector3(0,0,diagLength-2);zAxis.arrow=true;this.glScene.add(zAxis);this.axisVisible=true;}}
