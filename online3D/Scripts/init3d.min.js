...
this.initGL=function(container){var HEIGHT=$(window).height();var WIDTH=$("#3DArea").width();var VIEW_ANGLE=50,ASPECT=WIDTH/HEIGHT,NEAR=0.1,FAR=1000;this.glRenderer=new THREE.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});this.glCamera=new THREE.PerspectiveCamera(VIEW_ANGLE,ASPECT,NEAR,FAR);this.glCamera.updateProjectionMatrix();this.glScene=new THREE.Scene();this.glScene.add(this.glCamera);TOOLS.attach(this.glScene);this.glRenderer.setSize(WIDTH,HEIGHT);this.glRenderer.domElement.id="canvas";container.append(this.glRenderer.domElement);$("#3DArea #canvas").css("position","absolute");$("#3DArea #canvas").css("z-index","2");$("#3DArea #canvas").css("top","45px");$(window).resize(function(){_this.glCamera.aspect=window.innerWidth/window.innerHeight;_this.glCamera.updateProjectionMatrix();_this.glRenderer.setSize(window.innerWidth,window.innerHeight);});$("#3DArea").click(function(e){if(e.button===0)
{var pointClicked=TOOLS.getVertexFromMouseCoord(e);if(pointClicked!==undefined){_this.sceneTracker.center=pointClicked;_this.spotlight.position=_this.glCamera.position;}}});}};init.prototype.extensionIsOk=function(file){return true;}
