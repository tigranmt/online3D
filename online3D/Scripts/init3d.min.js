...
﻿function init(){this.glScene=undefined;this.glRenderer=undefined;this.glCamera=undefined;this.sceneTracker=undefined;this.modelLoader=undefined;this.textMesh=undefined;var _this=this;this.pointFromRay=function(event){var vector=new THREE.Vector3((event.clientX/$("#3DArea").width())*2-1,-(event.clientY/window.innerHeight)*2+1,0.5);var projector=new THREE.Projector();projector.unprojectVector(vector,_this.glCamera);var ray=new THREE.Ray(_this.glCamera.position,vector.subSelf(_this.glCamera.position).normalize());var point=undefined;_this.glScene.forEachMesh(function(mesh){var objects=new Array();objects.push(mesh);var intersects=ray.intersectObjects(objects,false);if(intersects.length>0){point=intersects[0].point;return point;}},function continueIteration(mesh){if(point!==undefined)
return false;if(_this.isComposedMesh(mesh)){return true;}});return point;}
...
return boundingBox;}
$("#3DArea").click(function(e){if(e.button===1)
{var pointClicked=_this.pointFromRay(e);if(pointClicked!==undefined)
_this.sceneTracker.center=pointClicked;}});}};init.prototype.isComposedMesh=function(mesh){return(mesh.children!=undefined&&mesh.children.length==2&&mesh.children[0].geometry!=undefined);}
...
tempModel.Vertices=tempModel.Vertices.concat(data.Vertices);packet++;load();}
...
var center=this.lookFrom(vector);var spotlight=new THREE.SpotLight(0xFFFFFF,0.8);spotlight.position=camera.position;spotlight.target.position=center;scene.add(spotlight);}
