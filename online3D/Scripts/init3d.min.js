...
this.pointFromRay=function(event){var x=(event.clientX/window.innerWidth)*2-1;var y=-((event.clientY-$("#navbar").height())/window.innerHeight)*2+1;var scenePosition=_this.sceneTracker.center;var viewDirection=new THREE.Vector3();viewDirection.set(x,y,1);var projector=new THREE.Projector();projector.unprojectVector(viewDirection,_this.glCamera);viewDirection=viewDirection.sub(viewDirection,scenePosition);var objects=[];_this.glScene.forEachMesh(function(mesh){objects.push(mesh.children[0]);},_this.isComposedMesh);var raycaster=new THREE.Raycaster(_this.glCamera.position,viewDirection.normalize());var intersects=raycaster.intersectObjects(objects);var point=new THREE.Vector3();if(intersects.length>0){point=intersects[0].point;return point;}}
