...
﻿function init(){this.glScene=undefined;this.glRenderer=undefined;this.glCamera=undefined;this.sceneTracker=undefined;this.modelLoader=undefined;this.textMesh=undefined;var _this=this;this.drawLine=function(scene,start,end){var geometry=new THREE.Geometry();geometry.vertices.push(start);geometry.vertices.push(end);var material=new THREE.LineBasicMaterial({color:0x0000ff,});var line=new THREE.Line(geometry,material);scene.add(line);finf}
...
{var pointClicked=TOOLS.getVertexFromMouseCoord(e);if(pointClicked!==undefined){_this.sceneTracker.target=pointClicked;_this.spotlight.position=_this.glCamera.position;_this.spotlight.target.position=pointClicked;}}});}};init.prototype.extensionIsOk=function(file){return true;}
init.prototype.setupSceneTracking=function(){this.sceneTracker=new THREE.TrackballControls(this.glCamera,$("#3DArea")[0]);this.sceneTracker.addEventListener('change',this.render);TOOLS.attach(this.glScene,this.glCamera,this.sceneTracker);}
init.prototype.loadMeshesInformation=function(){var _count=this.glScene.__objects.length;var infoViewModels=new Array();var imIndex=0;var _this=this;var sessioninfo=undefined;TOOLS.forEachMesh(function(mesh){function modelInfo(){var self=this;self.visible=ko.observable(true);if(mesh.name.length>25)
...
init.prototype.sendModelsToServer=function(sessionInfo){var unique=undefined;var meshIndex=0;var _this=this;(function uploadSingleModel(){var childrenCount=_this.glScene.__objects.length;if(meshIndex>=childrenCount){$("#flprogress").remove();toastr.success('Upload of all models done.','Success !');return;}
var mesh=_this.glScene.__objects[meshIndex];if(!TOOLS.isComposedMesh(mesh)){meshIndex++;uploadSingleModel();return;}
...
break;}
var modelInfo={ModelName:mesh.name,Size:mesh.filesize,Format:"Stl",ID:unique,Vertices:verticesSplit,VertexCount:verticesCount,Color:mesh.color,FaceColors:colorsSplit,SessionName:sessionInfo.sessionName,};if(index===1&&!firstLoad){modelInfo.ModelImage=_this.takeScreenshot();modelInfo.Notes=notesmodel.forJSON();firstLoad=true;}
...
else{meshIndex++;_this.showUploadProgress(mesh.name,100);uploadSingleModel();}},error:function(data){toastr.error('Error on save.','Error !');$("#flprogress").remove();},statusCode:{401:function(data){meshIndex=-1;$("#flprogress").remove();toastr.error('You need to login first.','Error !');userAccess.requestUserAuth();}}});};currentModelSequence();})();}
init.prototype.sendContentToServer=function(){var _this=this;jQuery('<div/>',{id:'session',class:'modal hide fade gridbody'}).appendTo($("#body"));var sessionInfo={};var session=$("#session");session.html(" <div class='modal-header'>"+"<h3>Set a name to session</h3>"+"</div>"+"<div class='modal-body'>"+"<input id='sessionnametext' type='text' style='width: 35em;' maxlength='50' placeholder='Type something…'>"+"</div>"+"<div class='modal-footer'>"+"<button id='startsharing' type='button' data-dismiss='modal' class='btn btn-success btn-medium'>Share</button>"+"<button id='cancelsharing' type='button' data-dismiss='modal' class='btn btn-danger btn-medium'>Cancel</button>"+"</div>");$("#startsharing").click(function(){sessionInfo.sessionName=$("#sessionnametext")[0].value;session.modal('hide');session.remove();_this.sendModelsToServer(sessionInfo);});$("#cancelsharing").click(function(){session.modal('hide');session.remove();return;});session.modal();}
init.prototype.LoadFromServer=function(unique){var _this=this;var mindex=0;var packet=0;var models=new Array();(function load(){$.ajax({url:'../GetModels/',data:{id:unique,modelIndex:mindex,packetIndex:packet},success:function(data){if(tempModel.ModelName===undefined)
...
tempModel.FaceColors=data.FaceColors;if(tempModel.SessionName===undefined)
tempModel.SessionName=data.SessionName;if(tempModel.User===undefined)
tempModel.User=data.User;if(tempModel.SavedOn===undefined)
tempModel.SavedOn=data.SavedOn;if(mindex==0&&tempModel.Notes===undefined){tempModel.Notes=data.Notes}
if(data==="modeldone"){models.push(tempModel);mindex++;packet=0;tempModel={};_this.showDownloadProgress(tempModel.ModelName,100);load();}
...
init.prototype.finalizeLoading=function(meshes){this.loadMeshesInformation.apply(this);if(meshes&&meshes.length>0){var first=meshes[0];var si=viewmodels.sessioninfo;si.sessionName=first.SessionName;si.userName=first.User;si.uploadDate=firs.SavedOn;}}
init.prototype.removeAxis=function(){var meshesToRemove=[];TOOLS.forEachMesh(function(mesh){meshesToRemove.push(mesh);},function(mesh){return mesh.arrow!==undefined;});for(var i=0;i<meshesToRemove.length;i++)
TOOLS.removeMesh(meshesToRemove[i]);}
init.prototype.drawAxis=function(scene,length){var cylinder=new THREE.CylinderGeometry(0.06,0.06,length,28,28);var cone=new THREE.CylinderGeometry(0.18,0.01,1,16,16);var materialx=new THREE.MeshPhongMaterial({color:0xFF0000});var xAxis=new THREE.Mesh(cylinder,materialx);var conex=new THREE.Mesh(cone,materialx);conex.position.y+=-length/2;var compositex=new THREE.Object3D();compositex.add(xAxis);compositex.add(conex);compositex.rotation.z=90*Math.PI/180;compositex.position.x+=length/2;compositex.arrow=true;scene.add(compositex);var materialy=new THREE.MeshPhongMaterial({color:0x00FF00});var yAxis=new THREE.Mesh(cylinder,materialy);var coney=new THREE.Mesh(cone,materialy);coney.position.y+=length/2;coney.rotation.x=Math.PI;var compositey=new THREE.Object3D();compositey.add(yAxis);compositey.add(coney);compositey.position.y+=length/2;compositey.arrow=true;scene.add(compositey);var materialz=new THREE.MeshPhongMaterial({color:0x0000FF});var zAxis=new THREE.Mesh(cylinder,materialz);var conez=new THREE.Mesh(cone,materialz);conez.position.y+=length/2;conez.rotation.x=Math.PI;var compositez=new THREE.Object3D();compositez.add(zAxis);compositez.add(conez);compositez.rotation.x=90*Math.PI/180;compositez.position.z+=length/2;compositez.arrow=true;scene.add(compositez);var centerMesh=new THREE.Mesh(new THREE.SphereGeometry(0.15,18,18),new THREE.MeshPhongMaterial({color:0x111111}));centerMesh.arrow=true;scene.add(centerMesh);}
init.prototype.showAxis=function(){if(this.axisVisible){this.removeAxis();this.axisVisible=false;}
else{var bounds=TOOLS.getSceneBoundingBox();var diag=new THREE.Vector3();diag=diag.subVectors(bounds.max,bounds.min);var diagLength=diag.length();this.drawAxis(this.glScene,diagLength);var shapex=new THREE.TextGeometry("X",{font:'helvetiker',height:0.5,size:2});var wrapperx=new THREE.MeshNormalMaterial({color:0x111111});var xAxis=new THREE.Mesh(shapex,wrapperx);xAxis.position=new THREE.Vector3(diagLength-2,0,0);xAxis.arrow=true;this.glScene.add(xAxis);var shapey=new THREE.TextGeometry("Y",{font:'helvetiker',height:0.5,size:2});var wrappery=new THREE.MeshNormalMaterial({color:0x111111});var yAxis=new THREE.Mesh(shapey,wrappery);yAxis.position=new THREE.Vector3(0,diagLength-2,0);yAxis.arrow=true;this.glScene.add(yAxis);var shapez=new THREE.TextGeometry("Z",{font:'helvetiker',height:0.5,size:2});var wrapperz=new THREE.MeshNormalMaterial({color:0x111111});var zAxis=new THREE.Mesh(shapez,wrapperz);zAxis.position=new THREE.Vector3(0,0,diagLength-2);zAxis.arrow=true;this.glScene.add(zAxis);this.axisVisible=true;}}
...
if(this.directionalLight===undefined){this.directionalLight=new THREE.DirectionalLight(0xffffff,0.1);this.directionalLight.position.set(camera.position.x,camera.position.y,camera.position.z);scene.add(this.directionalLight);}
if(this.ambientLight===undefined){this.ambientLight=new THREE.AmbientLight(0x444444);scene.add(this.ambientLight);}
...
return;var _this=this;_this.initScene(_this);var index=meshes.length-1;var notes=new Array();var loadAsync=function(){if(index<0){_this.fitCamera.apply(_this,[_this.glScene,_this.glCamera,_this.sceneTracker]);_this.renderOnScreen();_this.finalizeLoading.apply(_this,meshes);_this.showPanels();for(var i=0;i<notes.length;i++){var note=notes[i];notesmodel.addNoteToList(note.NoteText,i+1,note.NoteVertex);}
if(notes.length>0)
notesmodel.expand();toastr.success('Done !');return;}
var mesh=meshes[index];if(mesh.Notes!==undefined)
notes=mesh.Notes;_this.modelLoader.loadMeshModel(mesh,loadAsync);index--;};loadAsync();}
