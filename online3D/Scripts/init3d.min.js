...
this.pointFromRay=function(event){var x=(event.clientX/window.innerWidth)*2-1;var y=-((event.clientY-$("#navbar").height())/window.innerHeight)*2+1;var cameraPosition=_this.glCamera.position;var viewDirection=new THREE.Vector3();viewDirection.set(x,y,1);var projector=new THREE.Projector();projector.unprojectVector(viewDirection,_this.glCamera);viewDirection=viewDirection.subVectors(viewDirection,cameraPosition);var objects=[];_this.glScene.forEachMesh(function(mesh){objects.push(mesh.children[0]);},_this.isComposedMesh);viewDirection.normalize();this.drawLine(_this.glScene,cameraPosition,viewDirection);var raycaster=new THREE.Raycaster(cameraPosition,viewDirection);var intersects=raycaster.intersectObjects(objects);var point=new THREE.Vector3();if(intersects.length>0){point=intersects[0].point;return point;}}
...
init.prototype.hideShowModel=function(modelName){this.unlightModels();var breakExecution=false;this.glScene.forEachMesh(function(mesh){if(mesh.name===modelName){if(mesh.visible===undefined)
...
return false;return _this.isComposedMesh(mesh);});return result;}
init.prototype.higlightModel=function(modelName){_this.glScene.forEachMesh(function(mesh){var mat=mesh.children[0].material;if(mesh.name!==modelName){mat.opacity=0.6;}
else{mat.opacity=1.0;}
mat.needsUpdate=true;},_this.isComposedMesh);}
init.prototype.unlightModels=function(modelName){_this.glScene.forEachMesh(function(mesh){var mat=mesh.children[0].material;mat.opacity=1.0;mat.needsUpdate=true;},_this.isComposedMesh);}
this.initGL=function(container){var HEIGHT=$(window).height();var WIDTH=$("#3DArea").width();var VIEW_ANGLE=50,ASPECT=WIDTH/HEIGHT,NEAR=0.1,FAR=1000;this.glRenderer=new THREE.WebGLRenderer({antialias:true,preserveDrawingBuffer:true});this.glCamera=new THREE.PerspectiveCamera(VIEW_ANGLE,ASPECT,NEAR,FAR);this.glCamera.updateProjectionMatrix();this.glScene=new THREE.Scene();this.glScene.add(this.glCamera);this.glScene.forEachMesh=function(callback,canProcessCallback){var childrenCount=this.__objects.length;for(var i=0;i<childrenCount;i++){var mesh=this.__objects[i];if(mesh!==undefined&&(canProcessCallback===undefined||canProcessCallback.call(this,mesh)!==false)){callback.call(this,mesh);}}};this.glRenderer.setSize(WIDTH,HEIGHT);this.glRenderer.domElement.id="canvas";container.append(this.glRenderer.domElement);$("#3DArea #canvas").css("position","absolute");$("#3DArea #canvas").css("z-index","2");$("#3DArea #canvas").css("top","45px");$(window).resize(function(){_this.glCamera.aspect=window.innerWidth/window.innerHeight;_this.glCamera.updateProjectionMatrix();_this.glRenderer.setSize(window.innerWidth,window.innerHeight);});this.glScene.getSceneBoundingBox=function(){var boundingBox={min:new THREE.Vector3(Number.MAX_VALUE),max:new THREE.Vector3(Number.MIN_VALUE)};this.forEachMesh(function(mesh){var geometry=mesh.geometry;if(geometry===undefined)
return;geometry.computeBoundingBox();var bounds=geometry.boundingBox;boundingBox.min.x=Math.min(bounds.min.x,boundingBox.min.x);boundingBox.min.y=Math.min(bounds.min.y,boundingBox.min.y);boundingBox.min.z=Math.min(bounds.min.z,boundingBox.min.z);boundingBox.max.x=Math.max(bounds.max.x,boundingBox.max.x);boundingBox.max.y=Math.max(bounds.max.y,boundingBox.max.y);boundingBox.max.z=Math.max(bounds.max.z,boundingBox.max.z);})
return boundingBox;}
$("#3DArea").click(function(e){if(e.button===0)
{var pointClicked=_this.pointFromRay(e);if(pointClicked!==undefined){_this.sceneTracker.center=pointClicked;_this.spotlight.position=_this.glCamera.position;}}});}};init.prototype.isComposedMesh=function(mesh){return(mesh.children!=undefined&&mesh.children.length==2&&mesh.children[0].geometry!=undefined);}
init.prototype.extensionIsOk=function(file){return true;}
...
init.prototype.loadMeshesInformation=function(){var _count=this.glScene.__objects.length;var infoViewModels=new Array();var imIndex=0;var _this=this;this.glScene.forEachMesh(function(mesh){function modelInfo(){var self=this;self.visible=ko.observable(true);if(mesh.name.length>25)
...
self.isVisible=function(){return self.visible();};};infoViewModels.push(new modelInfo());imIndex++;},this.isComposedMesh);var infos=new(function(infos){this.modelsInformation=infos;})(infoViewModels);var infolist=$("#infoList")[0];ko.applyBindings(infos,infolist);}
...
var mesh=_this.glScene.__objects[meshIndex];if(!_this.isComposedMesh(mesh)){meshIndex++;uploadAnotherModel();return;}
...
var screenShotTaken=false;function currentModelSequence(){var verticesSplit=new Array();while(index<verticesCount){var gv=geometryMesh.geometry.vertices[index];var vertex="x:"+parseFloat(Math.round(gv.x*100)/100).toFixed(2)+" "+"y:"+parseFloat(Math.round(gv.y*100)/100).toFixed(2)+" "+"z:"+parseFloat(Math.round(gv.z*100)/100).toFixed(2);verticesSplit.push(vertex);index++;if(index==1)
break;if(verticesSplit.length%step===0)
break;}
var modelInfo={ModelName:mesh.name,Size:mesh.filesize,Format:"Stl",ID:unique,Vertices:verticesSplit,VertexCount:verticesCount,Color:mesh.color,};if(index===1&&!screenShotTaken){modelInfo.ModelImage=_this.takeScreenshot();screenShotTaken=true;}
...
tempModel.Color=data.Color;if(data==="modeldone"){models.push(tempModel);mindex++;packet=0;tempModel={};_this.showDownloadProgress(tempModel.ModelName,100);load();}
...
var center=this.lookFrom(vector);if(this.spotlight===undefined){this.spotlight=new THREE.SpotLight(0xFFFFFF,0.8);scene.add(this.spotlight);}
...
init.prototype.solidView=function(meshname){this.glScene.forEachMesh(function(mesh){if(!mesh.visible)
return;if(meshname!==undefined&&meshname!==mesh.name)
return;mesh.children[0].visible=true;mesh.children[1].visible=false;},this.isComposedMesh);}
init.prototype.wireframeView=function(meshname){this.glScene.forEachMesh(function(mesh){if(!mesh.visible)
return;if(!mesh.visible)
return;if(meshname!==undefined&&meshname!=mesh.name)
return;mesh.children[0].visible=false;mesh.children[1].visible=true;},this.isComposedMesh);}
init.prototype.meshView=function(){this.glScene.forEachMesh(function(mesh){if(!mesh.visible)
return;mesh.children[0].visible=true;mesh.children[1].visible=true;},this.isComposedMesh);}
