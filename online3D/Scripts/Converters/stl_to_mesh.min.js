...
var complete=function(){if(geometry.vertices.length>0){var meshMaterial=new THREE.MeshPhongMaterial({ambient:0x222222,vertexColors:THREE.FaceColors,specular:0x49D8FB,shininess:140,perPixel:false,overdraw:false});var meshWireframe=new THREE.MeshBasicMaterial({color:0x555555,wireframe:true});var multiMaterial=[meshMaterial,meshWireframe];geometry.computeFaceNormals();geometry.computeVertexNormals();geometry.__dirtyColors=true;var mesh=THREE.SceneUtils.createMultiMaterialObject(geometry,multiMaterial);mesh.name=fileName;mesh.facecount=geometry.faces.length;mesh.verticescount=geometry.vertices.length;mesh.filesize=Math.round((fileSize/1024)*100)/100;mesh.color=modelColor;_scene.add(mesh);}
_finishCallback();}
...
iterator();}
var complete=function(){if(geometry.vertices.length>0){var meshMaterial=new THREE.MeshPhongMaterial({ambient:0x222222,vertexColors:THREE.FaceColors,specular:0x49D8FB,shininess:140,perPixel:false,overdraw:true});var meshWireframe=new THREE.MeshBasicMaterial({color:0x555555,wireframe:true});var multiMaterial=[meshMaterial,meshWireframe];geometry.computeFaceNormals();geometry.computeVertexNormals();geometry.__dirtyColors=true;var mesh=THREE.SceneUtils.createMultiMaterialObject(geometry,multiMaterial);mesh.name=fileName;mesh.facecount=geometry.faces.length;mesh.verticescount=geometry.vertices.length;mesh.filesize=Math.round((fileSize/1024)*100)/100;mesh.color=modelColor;_scene.add(mesh);}
_finishCallback();}
async.until(isLoadingDone,readStep,complete);}}};