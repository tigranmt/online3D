...
function isPanState(event){if((_state===STATE.ROTATE&&event.button===STATE.PAN)||(_state===STATE.PAN&&event.button===STATE.ROTATE)){return true;}
return false;}
...
if(isPanState(event)){_panStart=_panEnd=_this.getMouseOnScreen(event.clientX,event.clientY);}
else{if(_state===STATE.ROTATE&&!_this.noRotate){_rotateStart=_rotateEnd=_this.getMouseProjectionOnBall(event.clientX,event.clientY);}else if(_state===STATE.ZOOM&&!_this.noZoom){_zoomStart=_zoomEnd=_this.getMouseOnScreen(event.clientX,event.clientY);}}
document.addEventListener('mousemove',mousemove,false);document.addEventListener('mouseup',mouseup,false);}
function mousemove(event){if(_this.enabled===false)return;event.preventDefault();event.stopPropagation();if(isPanState(event)){_panEnd=_this.getMouseOnScreen(event.clientX,event.clientY);}
else{if(_state===STATE.ROTATE&&!_this.noRotate){_rotateEnd=_this.getMouseProjectionOnBall(event.clientX,event.clientY);}else if(_state===STATE.ZOOM&&!_this.noZoom){_zoomEnd=_this.getMouseOnScreen(event.clientX,event.clientY);}}}
